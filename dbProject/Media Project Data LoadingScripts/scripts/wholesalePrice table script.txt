...
//Description: script for pushing data into wholesalePrice table
...
$json = file_get_contents("http://api.randomuser.me/?results=30");
//print_r($json);
...
for($i=0;$i<sizeof($json_array->results);$i++)
{
    ...
    //killing the sql with an error
        ...
        $j1=rand(1.00,20.00);
        $start = strtotime("2012-9-01 00:00:00");
        $end =  strtotime("2015-10-31 23:59:59");
        $randomDate = date("Y-m-d H:i:s", rand($start, $end));
        $sql1 = "
			INSERT INTO wholesalePrice
			VALUES('$pid','$j1','$j0','$randomDate')";
            $j0=$j0-1;
