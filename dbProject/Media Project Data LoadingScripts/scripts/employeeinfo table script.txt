...
//Description: script for pushing data into employeeinfo table
//sudha mallavarapu database was used to do the scripts for the credentials
//################################################################
...
for($i=0;$i<sizeof($json_array->results);$i++)
{
    ...
    //killing the sql with an error
            ...
            while($row = $value->fetch_assoc()) 
                {
                    ...
                    if($rid==2)
                    {
                    $pay=rand(10.00,20.00);
                    
                    $ssn=$phone.$rid;
                    $sql1 = "
                        INSERT INTO employeeInfo
                        VALUES('$ssn','$d','$pay','$uuid')";
       ...
       //$sql1 = "
		//	INSERT INTO users
			//VALUES('$first','$last','$email','$password','$street','$city','$state','$zip','$phone','$uuid','$rid')";
        $uuid=$uuid+1;    
		//#echo $sql1."\n";	
        //throwing the error
